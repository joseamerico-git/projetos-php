    ...
    $('#exampleModal').on('show.bs.modal', function (event) {
          ...
          var recipient = button.data('whatever') // Extract info from data-* attributes
           ...
           var recipient1 = button.data('whatevernome') // Extract info from data-* attributes
            ...
            var recipient2 = button.data('whateveremail') // Extract info from data-* attributes
            
            var recipient3 = button.data('whateversenha') // Extract info from data-* attributes
            
          // If necessary, you could initiate an AJAX request here (and then do the updating in a callback).
          // Update the modal's content. We'll use jQuery here, but you could use a data binding library or other methods instead.
          var modal = $(this)
          modal.find('.modal-title').text('Alterar Usuario - '+ recipient )
         
          
  
          modal.find('#cd').val(recipient)
          modal.find('#n').val(recipient1)
          modal.find('#em').val(recipient2)
           modal.find('#se').val(recipient3)
